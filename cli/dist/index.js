...
// src/types/index.ts
var AI_TYPES = ["claude", "cursor", "windsurf", "antigravity", "copilot", "roocode", "kiro", "codex", "qoder", "gemini", "trae", "opencode", "continue", "codebuddy", "droid", "all"];
var AI_FOLDERS = {
  claude: [".claude"],
...
import { readFile, mkdir as mkdir2, writeFile, cp as cp2 } from "node:fs/promises";
...
async function copyExtraSkills(targetSkillsDir, extraSkills) {
  const copiedSkills = [];
  const extraSkillsSource = join2(ASSETS_DIR, "extra-skills");
  if (!await exists(extraSkillsSource)) {
    return copiedSkills;
  }
  for (const skillName of extraSkills) {
    const source = join2(extraSkillsSource, skillName);
    const target = join2(targetSkillsDir, skillName);
    if (await exists(source)) {
      await mkdir2(target, { recursive: true });
      await cp2(source, target, { recursive: true });
...
async function generatePlatformFiles(targetDir, aiType) {
  ...
  if (config.extraSkills && config.extraSkills.length > 0) {
    ...
    const skillsDir = join2(targetDir, config.folderStructure.root, skillsParent);
    const copiedSkills = await copyExtraSkills(skillsDir, config.extraSkills);
    if (copiedSkills.length > 0) {
      createdFolders.push(...copiedSkills.map((s) => `${config.folderStructure.root}/${skillsParent}/${s}`));
    }
  }
...
function detectAIType(cwd = process.cwd()) {
  ...
  for (const [folder, aiType] of Object.entries(FOLDER_TO_AI_TYPE)) {
    if (existsSync(join3(cwd, folder))) {
